...
do  
    ...
    #java -jar Mars.jar perfect.asm < "$TEST_DIRECTORY$program_input" | tail -n +2  > "$OUTPUT_DIRECTORY$output_filename"
    spim -f perfect.asm < "$TEST_DIRECTORY$program_input" | tail -n +2  > "$OUTPUT_DIRECTORY$output_filename"
    ########################################################################################
    if [[ $? -eq 0 ]]; then
